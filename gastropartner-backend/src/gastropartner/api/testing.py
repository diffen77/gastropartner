...
class TestingService:
    ...
    def __init__(self):
        try:
            ...
            print(f"Error initializing TestingService: {e}")
            # Fallback paths
    ...
    def parse_junit_xml(self, xml_path: Path) -> Optional[TestSuite]:
    ...
    def parse_coverage_json(self, coverage_path: Path) -> Optional[float]:
    ...
    async def get_backend_results(self) -> Optional[TestSuite]:
    ...
    async def get_frontend_results(self) -> Optional[TestSuite]:
...
async def get_backend_test_results():
    ...
    if results is None:
        raise HTTPException(
            status_code=404, 
            detail="No backend test results found. Run tests first."
        )
    
    # Add coverage if available
    coverage = testing_service.parse_coverage_json(
        testing_service.reports_dir / "coverage.json"
    )
    if coverage is not None:
...
async def get_frontend_test_results():
    ...
    if results is None:
        raise HTTPException(
            status_code=404,
            detail="Frontend test results not available yet."
        )
    return results
...
async def run_tests(
    ...
    if test_type not in ['backend', 'frontend', 'all']:
        raise HTTPException(
            status_code=400,
            detail="test_type must be 'backend', 'frontend', or 'all'"
        )
    ...
    return {
        "message": f"Started running {test_type} tests",
        "status": "running"
    }
