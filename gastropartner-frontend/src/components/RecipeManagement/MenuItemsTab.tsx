...
export function MenuItemsTab({ isActive }: TabContentProps) {
  ...
  const handleSubmitMenuItem = async (data: MenuItemCreate) => {
    try {
      ...
      // Use context's menu item change handler to update all related data
      await onMenuItemChange();
      setError('');
    } catch (err) {
