...
class BackendAgent:
    ...
    async def validate(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if file_ext == ".py":
            results.extend(self._validate_python_code(file_path, file_content, validation_rules))
            
            # FastAPI specific validation
            if self._is_fastapi_file(file_content):
                results.extend(self._validate_fastapi_patterns(file_path, file_content, validation_rules))
            
            # Database file validation
            if self._is_database_file(file_path, file_content):
                results.extend(self._validate_database_patterns(file_path, file_content, validation_rules))
            
            # AST-based validation
            results.extend(await self._validate_python_ast(file_path, file_content, validation_rules))
    ...
    def _validate_python_code(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
    ...
    def _validate_fastapi_patterns(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
    ...
    def _validate_database_patterns(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if validation_rules.get("database", {}).get("require_organization_filter", True):
            has_db_operations = any(op in file_content.lower() for op in ["select", "update", "delete", "insert"])
    ...
    async def _validate_python_ast(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        ...
        try:
            ...
            max_function_length = validation_rules.get("python", {}).get("max_function_length", 50)
            ...
            # Check class length
            max_class_length = validation_rules.get("python", {}).get("max_class_length", 100)
            for node in ast.walk(tree):
    ...
    def _validate_type_hints(self, tree: ast.AST, file_path: str) -> List[ValidationResult]:
    ...
    def _validate_function_complexity(self, tree: ast.AST, file_path: str) -> List[ValidationResult]:
    ...
    def _validate_route_organization(self, file_path: str, file_content: str) -> List[ValidationResult]:
        """Validate FastAPI route organization."""
        results = []
        
        # Check for route grouping
        route_patterns = re.findall(r'@\w+\.(get|post|put|delete)\s*\(["\']([^"\']+)["\']', file_content)
    ...
    def _validate_pydantic_models(self, file_path: str, file_content: str) -> List[ValidationResult]:
        """Validate Pydantic model usage."""
        results = []
        
        # Check for Pydantic model definitions
        has_models = "BaseModel" in file_content
        has_routes = any(method in file_content for method in ["@app.post", "@app.put", "@router.post", "@router.put"])
    ...
    def get_backend_quality_report(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        categories = {
            ...
            "complexity_issues": [r for r in backend_results if "complexity" in r.rule_id or "length" in r.rule_id],
            "type_hint_issues": [r for r in backend_results if "type_hint" in r.rule_id]
        }
        ...
        return {
            ...
            "severity_breakdown": {
                ...
                "warnings": len([r for r in backend_results if r.severity == "warning"]),
    ...
    def _calculate_quality_score(self, results: List[ValidationResult]) -> int:
        ...
        weighted_score = sum([
            error_weight if r.severity == "error" else
            warning_weight if r.severity == "warning" else
            info_weight
            for r in results
        ])
        ...
        quality_score = max(0, 100 - (weighted_score / max_score * 100)) if max_score > 0 else 100
        
        return int(quality_score)
    
    def _generate_backend_recommendations(self, results: List[ValidationResult]) -> List[str]:
        ...
        if rule_counts.get("missing_type_hint", 0) > 5:
            recommendations.append("Add comprehensive type hints throughout the codebase")
        
        if rule_counts.get("function_too_long", 0) > 2:
            recommendations.append("Refactor long functions into smaller, more focused functions")
        
        if rule_counts.get("high_complexity", 0) > 1:
            recommendations.append("Reduce cyclomatic complexity by simplifying control flow")
        
        if rule_counts.get("missing_organization_filter", 0) > 0:
            recommendations.append("Ensure all database operations include organization_id filtering")
        
        if rule_counts.get("route_organization", 0) > 0:
            recommendations.append("Consider using APIRouter for better route organization")
