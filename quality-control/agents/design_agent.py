...
class DesignAgent:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.design_tokens = {
            ...
            "typography": [
                "heading", "body", "caption", "label", "code", "display"
            ],
            "breakpoints": [
                "mobile", "tablet", "desktop", "wide", "sm", "md", "lg", "xl"
            ]
        }
    ...
    async def validate(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        ...
        if file_ext in [".css", ".scss", ".sass"]:
            results.extend(self._validate_css_file(file_path, file_content, validation_rules))
        
        # React/HTML component validation for inline styles and class names
        elif file_ext in [".tsx", ".jsx", ".html"]:
            results.extend(self._validate_component_styles(file_path, file_content, validation_rules))
        
        # Global validation for design token consistency
        results.extend(self._validate_design_token_usage(file_path, file_content, validation_rules))
        
        return results
    
    def _validate_css_file(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        ...
        results.extend(self._validate_css_specificity(file_path, file_content, validation_rules))
    ...
    def _validate_component_styles(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
    ...
    def _validate_bem_naming(self, file_path: str, file_content: str) -> List[ValidationResult]:
    ...
    def _is_valid_bem_class(self, class_name: str) -> bool:
        ...
        utility_prefixes = ["flex", "grid", "text-", "bg-", "p-", "m-", "w-", "h-", "btn", "card"]
    ...
    def _validate_css_specificity(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
    ...
    def _validate_responsive_patterns(self, file_path: str, file_content: str) -> List[ValidationResult]:
    ...
    def _validate_design_token_usage(self, file_path: str, file_content: str, validation_rules: Dict[str, Any]) -> List[ValidationResult]:
        """Validate usage of design tokens."""
        results = []
        
        if not validation_rules.get("components", {}).get("require_design_tokens", True):
            return results
        ...
        for pattern_obj in hardcoded_patterns:
            ...
            for line_num, line in enumerate(lines, 1):
                if pattern.search(line):
                    ...
                    if not any(token_indicator in line for token_indicator in ["var(", "$", "--"]):
                        results.append(ValidationResult(
                            agent_type="design",
                            file_path=file_path,
                            severity="info",
    ...
    def extract_design_tokens(self, file_content: str) -> DesignTokens:
        ...
        for var in css_vars:
            ...
            elif any(spacing_token in var for spacing_token in self.design_tokens["spacing"]):
                tokens.spacing.add(var)
            elif any(typo_token in var for typo_token in self.design_tokens["typography"]):
    ...
    def get_design_consistency_report(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        categories = {
            ...
            "accessibility_issues": [r for r in design_results if "accessibility" in r.rule_id],
            "design_token_issues": [r for r in design_results if "design_token" in r.rule_id],
            "css_quality": [r for r in design_results if "css" in r.rule_id],
            "responsive_issues": [r for r in design_results if "responsive" in r.rule_id]
        }
    ...
    def _generate_design_recommendations(self, results: List[ValidationResult]) -> List[str]:
        ...
        if rule_counts.get("bem_violation", 0) > 5:
            recommendations.append("Establish and document BEM naming convention standards")
