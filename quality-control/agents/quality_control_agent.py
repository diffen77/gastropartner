...
# Import ML adaptation agent for data collection
from agents.ml_adaptation_agent import (
    MLAdaptationAgent,
...
class QualityControlAgent:
    ...
    def __init__(self, config_path: str = None, ml_agent: MLAdaptationAgent = None):
        self.console = Console()
        self.config_path = (
            config_path or Path(__file__).parent.parent / "config" / "agent_config.yaml"
        )
        self.validation_rules_path = (
            Path(__file__).parent.parent / "config" / "validation_rules.yaml"
        )

        # Initialize ML agent for data collection
        self.ml_agent = ml_agent or MLAdaptationAgent()
    ...
    def _initialize_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            agents["security"] = SecurityAgent(
                self.config.get("agents", {}).get("security_agent", {})
            )
            agents["functional"] = FunctionalAgent(
                self.config.get("agents", {}).get("functional_agent", {})
            )
            agents["design"] = DesignAgent(
                self.config.get("agents", {}).get("design_agent", {})
            )
            agents["backend"] = BackendAgent(
                self.config.get("agents", {}).get("backend_agent", {})
            )
            agents["localization"] = LocalizationAgent(
                self.config.get("agents", {}).get("localization_agent", {})
            )

            self.console.print(
                "[green]✓[/green] All sub-agents initialized successfully"
            )
    ...
    def _determine_validation_types(self, file_path: str) -> List[str]:
        ...
        if extension in [".tsx", ".ts", ".jsx", ".js"]:
            validation_types.extend(["functional", "localization"])
        ...
        # Config files get all validations
            ...
            file_path.name in ["package.json", "pyproject.toml"]
            or ".config." in file_path.name
        ):
            validation_types.extend(["functional", "backend"])

        # Localization files get special localization validation
        if "localization" in str(file_path).lower() or "i18n" in str(file_path).lower():
            validation_types.append("localization")

        return list(set(validation_types))  # Remove duplicates

    async def validate_file(
        self, file_path: str, validation_types: List[str] = None
        ...
        # Record validation results for ML analysis and continuous learning
        self._record_ml_validation_results(file_path, results, validation_types)

        return results

    def _record_ml_validation_results(
        self,
        file_path: str,
        results: List[ValidationResult],
        validation_types: List[str],
    ):
        """Record validation results in ML database for continuous learning."""
        try:
            # Convert each validation result to ML format and record it
            for result in results:
                # Determine success based on severity
                success = result.severity != "error"
                ...
                # Determine pattern type from agent type and file extension
                file_ext = Path(file_path).suffix.lower()
                pattern_type = (
                    f"{result.agent_type}_{file_ext}" if file_ext else result.agent_type
                )

                # Create ML validation result
                ml_result = MLValidationResult(
                    file_path=file_path,
                    rule_name=result.rule_id or f"{result.agent_type}_general",
                    pattern_type=pattern_type,
                    success=success,
                    confidence=confidence,
                    timestamp=datetime.now(),
                    error_type=result.severity if not success else None,
                    fix_applied=False,  # Could be enhanced to track if fixes are applied
                )

                # Record in ML database
                self.ml_agent.record_validation_result(ml_result)

            print(
                f"🧠 ML DATA: Recorded {len(results)} validation results for continuous learning"
            )

        except Exception as e:
            print(f"⚠️ ML RECORDING FAILED: {e}")
            # Continue without failing validation - data collection is secondary

    async def _run_validation(
        self, validation_type: str, file_path: str, file_content: str
    ) -> List[ValidationResult]:
        ...
        print(
            f"🤖 AGENT STARTING: {validation_type.upper()} validation on {Path(file_path).name}"
        )

        try:
            start_time = time.time()
            results = await agent.validate(
                file_path, file_content, self.validation_rules.get(validation_type, {})
            )
            ...
            if error_count > 0:
                print(
                    f"❌ AGENT COMPLETE: {validation_type.upper()} found {error_count} errors, {warning_count} warnings ({duration:.2f}s)"
                )
            elif warning_count > 0:
                print(
                    f"⚠️  AGENT COMPLETE: {validation_type.upper()} found {warning_count} warnings ({duration:.2f}s)"
                )
            else:
                print(
                    f"✅ AGENT COMPLETE: {validation_type.upper()} validation passed ({duration:.2f}s)"
                )
    ...
    async def validate_multiple_files(
        self, files: List[str], validation_types: List[str] = None
        ...
        semaphore = asyncio.Semaphore(
            self.config.get("concurrency", {}).get("max_agents", 3)
        )
    ...
    def format_results(
        self,
        results: Union[List[ValidationResult], Dict[str, List[ValidationResult]]],
    ...
    def _format_console_results(
        ...
        total_info = sum(
            len([r for r in file_results if r.severity == "info"])
            for file_results in results.values()
        )
    ...
    def _format_json_results(
        ...
        if isinstance(results, list):
            return json.dumps(
                [result.dict() for result in results], indent=2, default=str
            )
        else:
            return json.dumps(
                {
    ...
    def _format_markdown_results(
        ...
        for file_path, file_results in results.items():
            ...
            for result in file_results:
                ...
                output.append(
                    f"{emoji} **{result.severity.upper()}**: {result.message}"
                )
