...
class FunctionalAgent:
    ...
    async def validate(
        ...
        if file_ext in [".ts", ".tsx", ".js", ".jsx"]:
            results.extend(
                self._validate_typescript_functional(
            ...
            if file_ext in [".tsx", ".jsx"] or "component" in file_path.lower():
                results.extend(
                    self._validate_react_component(
                        file_path, file_content, validation_rules
                    )
                )
        ...
        # Package.json validation
            ...
            results.extend(
                self._validate_package_json(file_path, file_content, validation_rules)
            )

        # API-related files
        elif "api" in file_path.lower() or "service" in file_path.lower():
            results.extend(
                self._validate_api_patterns(file_path, file_content, validation_rules)
            )

        return results

    def _validate_typescript_functional(
        self, file_path: str, file_content: str, validation_rules: Dict[str, Any]
    ) -> List[ValidationResult]:
    ...
    def _validate_react_component(
        ...
        if validation_rules.get("react", {}).get("enforce_prop_types", True):
            results.extend(
                self._validate_prop_types(file_path, file_content, component_analysis)
            )
        ...
        if validation_rules.get("react", {}).get(
            "validate_state_synchronization", True
        ):
            results.extend(
                self._validate_react_synchronization(file_path, file_content)
            )
    ...
    def _analyze_component_structure(self, file_content: str) -> ComponentAnalysis:
        ...
        component_name_match = re.search(
            r"(?:function|const)\s+(\w+)(?:\s*[:=]|\s*\()", file_content
        )
        component_name = (
            component_name_match.group(1)
        ...
        import_matches = re.findall(
            r"import\s+.*?from\s+['\"]([^'\"]+)['\"]", file_content
        )
        ...
        interface_props = (
            re.findall(r"(\w+)\s*:\s*\w+", file_content)
            if "interface" in file_content
            else []
        )

        # Check if it has TypeScript
        has_typescript = (
            ".tsx" in file_content or "interface" in file_content or ":" in file_content
        )
    ...
    def _validate_prop_types(
        self, file_path: str, file_content: str, component_analysis: ComponentAnalysis
    ) -> List[ValidationResult]:
    ...
    def _validate_accessibility_basics(
        self, file_path: str, file_content: str
    ) -> List[ValidationResult]:
    ...
    def _validate_react_synchronization(
        self, file_path: str, file_content: str
    ) -> List[ValidationResult]:
    ...
    def _validate_imports(
        ...
        for line_num, import_line in imports_section:
            import_match = re.search(
                r"import\s+(?:{([^}]+)}|\*\s+as\s+(\w+)|(\w+))\s+from", import_line
            )
            if import_match:
