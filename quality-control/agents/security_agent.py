...
class SecurityAgent:
    ...
    async def validate(
        ...
        if file_ext == ".py":
            results.extend(
                await self._validate_python_security(
                    file_path, file_content, validation_rules
                )
            )
        elif file_ext in [".ts", ".tsx", ".js", ".jsx"]:
            results.extend(
                await self._validate_typescript_security(
                    file_path, file_content, validation_rules
                )
            )
        elif file_ext == ".sql":
            results.extend(
                self._validate_sql_security(file_path, file_content, validation_rules)
            )

        # Multi-tenant specific checks
        if validation_rules.get("multi_tenant", {}).get(
        ...
        try:
            ai_results = await self._run_ai_security_analysis(
                file_path, file_content, validation_rules
            )
        ...
        for pattern_obj in self.dangerous_patterns:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if (
                    ...
                    or "raise HTTPException" in line
                    or 'detail=f"' in line
                ):
                    continue
    ...
    def _check_organization_id_usage(
        self, file_path: str, file_content: str
    ) -> List[ValidationResult]:
        ...
        has_db_operations = any(
            keyword in file_content_upper for keyword in db_keywords
        )
    ...
    async def _validate_python_security(
        self, file_path: str, file_content: str, validation_rules: Dict[str, Any]
    ) -> List[ValidationResult]:
    ...
    async def _validate_typescript_security(
        self, file_path: str, file_content: str, validation_rules: Dict[str, Any]
    ) -> List[ValidationResult]:
        ...
        security_checks = [
            ...
            (
                r"document\.write\s*\(",
                "Deprecated document.write usage",
        ...
        if "fetch(" in file_content or "axios" in file_content:
            if (
                "authorization" not in file_content.lower()
                and "bearer" not in file_content.lower()
            ):
                results.append(
                    ValidationResult(
                        agent_type="security",
    ...
    def _validate_sql_security(
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if any(
                ...
                for pattern in ["DROP TABLE", "TRUNCATE", "DELETE FROM"]
            ):
            ...
            if (
                any(op in line_upper for op in ["SELECT", "UPDATE", "DELETE"])
                and "organization_id" not in line.lower()
            ):
    ...
    def get_security_summary(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        categories = {
            "multi_tenant": [
                r for r in security_results if "organization_id" in r.message.lower()
            ],
            "sql_injection": [
                r
                for r in security_results
                if "sql" in r.message.lower() and "injection" in r.message.lower()
            ],
            "hardcoded_secrets": [
                r
                for r in security_results
                if any(
                    word in r.message.lower() for word in ["password", "key", "secret"]
                )
            ],
            "xss": [
                r
                for r in security_results
                if "xss" in r.message.lower() or "innerhtml" in r.message.lower()
            ],
            "other": [],
        }
    ...
    def _generate_security_recommendations(
        ...
        if any("multitenant" in issue_id for issue_id in issue_types):
            recommendations.append(
                "Implement organization_id filtering in all database operations"
            )
    ...
    def _parse_ai_response(
        self, file_path: str, ai_response: str
    ) -> List[ValidationResult]:
        """Parse AI response and convert to ValidationResult objects."""
        results = []
        ...
        for line in lines:
            ...
            if line.lower().startswith(("error:", "warning:", "info:")):
                ...
                if current_issue and "message" in current_issue:
                    results.append(
                        self._create_validation_result_from_ai(file_path, current_issue)
                    )
    ...
    def _create_validation_result_from_ai(
        self, file_path: str, issue_data: dict
    ) -> ValidationResult:
