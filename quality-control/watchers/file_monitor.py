...
class FileMonitor:
    ...
    def __init__(self, quality_agent: QualityControlAgent, config: Dict[str, Any] = None):
        ...
        self.debounce_delay = self.config.get("debounce_delay", 0.1)  # AGGRESSIVE: ALWAYS validate immediately
        ...
        # Callbacks
        self.validation_callbacks: List[Callable[[str, List[ValidationResult]], None]] = []
    ...
    def _get_excluded_patterns(self) -> List[str]:
        ...
        default_excluded = [
            ...
            ".coverage", "dist", "build", ".next", ".vscode", "*.pyc",
    ...
    def add_validation_callback(self, callback: Callable[[str, List[ValidationResult]], None]):
    ...
    def queue_validation(self, file_path: str, change_type: str):
        ...
        with self.validation_lock:
            ...
            self.console.print(f"[cyan]📝 File changed:[/cyan] {Path(file_path).name} ({change_type})")
        ...
        try:
            ...
            self.console.print("[yellow]⚠️ Event loop not available, validation will be delayed[/yellow]")
    ...
    async def _validation_worker(self):
        ...
        while self.is_running:
            try:
                ...
                print(f"\n" + "="*60)
                print(f"🚨 PYDANTIC AI AGENTS RUNNING - QUALITY CONTROL ACTIVE 🚨")
                print(f"📄 FILE: {Path(file_path).name}")
                print(f"⏰ TIME: {time.strftime('%H:%M:%S')}")
                print(f"🔬 VALIDATING: Multi-tenant security, functionality, design...")
                print("="*60)
                self.console.print(f"[bold red]🚨 PYDANTIC AI AGENTS ACTIVE[/bold red]: {Path(file_path).name}")
                ...
                # Display results
                await self._display_validation_results(file_path, results, validation_time)
    ...
    async def _display_validation_results(self, file_path: str, results: List[ValidationResult], validation_time: float):
        ...
        if errors:
            ...
            print(f"📄 FILE: {file_name}")
            print(f"🟢 ALL CHECKS PASSED - SECURE & COMPLIANT")
        ...
        panel = Panel(
            table,
            title=f"{status} - {file_name}",
            border_style=color
        )
        # Print clear terminal summary
        if errors:
            print(f"❌ VALIDATION FAILED: {file_name} - {len(errors)} errors, {len(warnings)} warnings")
        elif warnings:
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.console.print("[yellow]⚠️ Async queue will be initialized when event loop is available[/yellow]")
        ...
        for watch_path in self.watch_paths:
            ...
            if path.exists():
                ...
                file_count = sum(1 for p in path.rglob("*") if p.is_file() and self._should_monitor_file(str(p)))
                self.stats["files_watched"] += file_count
                
                self.console.print(f"[green]👀 Watching:[/green] {watch_path} ({file_count} files)")
            else:
                self.console.print(f"[red]❌ Path not found:[/red] {watch_path}")
        
        self.console.print(f"[green]🚀 File monitoring started[/green] - Watching {len(self.observers)} directories")
    ...
    def display_status(self):
        ...
        table.add_row("Validations Triggered", str(status["statistics"]["validations_triggered"]))
        table.add_row("Validations Completed", str(status["statistics"]["validations_completed"]))
        table.add_row("Validations Failed", str(status["statistics"]["validations_failed"]))
