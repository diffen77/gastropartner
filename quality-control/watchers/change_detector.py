...
class ChangeDetector:
    ...
    def __init__(self, db_path: str = None):
        ...
        # Configuration
        self.validation_threshold_hours = (
            24  # Re-validate after 24 hours even if no changes
        )
    ...
    def setup_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_validation_file ON validation_history(file_path)"
            )
    ...
    def _should_validate(self, change: FileChange) -> bool:
        ...
        if last_validation:
            ...
            if (
                last_validation.file_hash == change.file_hash
                and last_validation.last_validated > datetime.now() - timedelta(hours=1)
            ):
                return False

            # Force validation if too much time has passed
            if last_validation.last_validated < datetime.now() - timedelta(
                hours=self.validation_threshold_hours
            ):
                return True
    ...
    def _is_significant_change(self, change: FileChange) -> bool:
        ...
        try:
            ...
            keyword_count = sum(
                content.lower().count(keyword) for keyword in significant_keywords
            )
    ...
    def get_frequently_changed_files(
        self, days: int = 7, min_changes: int = 5
    ) -> List[Tuple[str, int]]:
