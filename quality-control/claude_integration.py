...
class ClaudeIntegrationBridge:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.log_claude_message(
                "error", "Failed to initialize quality control", str(e)
            )
            raise
    ...
    async def validate_files(
        self, file_paths: List[str], validation_types: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        self.log_claude_message(
            "info",
            f"Validating {len(relevant_files)} files",
            f"Files: {', '.join(file_names)}",
        )

        # Log agent activity
        print("🛡️  SECURITY AGENT: Scanning for multi-tenant violations...")
        print("⚙️  BACKEND AGENT: Checking Python code quality and performance...")
        print("🎨 FRONTEND AGENT: Validating TypeScript/React patterns...")
        print("🔧 FUNCTIONAL AGENT: Testing business logic compliance...")
        print("🎯 DESIGN AGENT: Verifying UI consistency and accessibility...")
        ...
        try:
            ...
            validation_time = time.time() - start_time
            self.log_claude_message("error", "Validation failed with exception", str(e))
    ...
    def print_claude_feedback(self, feedback_result: Dict[str, Any]):
        ...
        if feedback_result.get("feedback"):
            ...
            claude_text = self.feedback_processor.format_for_claude(
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validations_per_minute": (self.validation_count / session_time * 60)
            if session_time > 0
            else 0,
...
async def main():
    ...
    try:
        ...
        if os.getenv("DEBUG", "").lower() in ("1", "true", "yes"):
            ...
            print(
                f"   Avg validation time: {perf_summary['performance_stats']['average_validation_time']:.2f}s"
            )
