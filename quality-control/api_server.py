...
class ValidationRequest(BaseModel):
    ...
    validation_types: Optional[List[str]] = Field(
        None, description="Specific validation types to run"
    )
    session_id: Optional[str] = Field(None, description="Session ID for tracking")
    priority: str = Field(
        "normal", description="Priority level: normal, high, critical"
    )
    ...
    def _setup_routes(self):
        ...
        async def health_check():
            ...
            return HealthResponse(
                ...
                active_agents=len(self.quality_agent.sub_agents)
        ...
        async def validate_files(
            ...
            try:
                ...
                validation_results = await self.quality_agent.validate_multiple_files(
                    request.files, request.validation_types
                )
                ...
                error_patterns = [
                    {
                        ...
                        if len(pattern.error_message) > 100
                        else pattern.error_message,
